                    ...
                    for (let i = 0; i < 3; i++) {
                        ...
                        if (result.data.length - 1 >= i) {
                            ...
                            if (item instanceof Anime) {
                                ...
                                card.append($("<a>").attr("class", "cyrus-card-link").attr("href", "www.google.com").attr("title", item.title)).append($("<div>").attr("class", "cyrus-card-image").append($("<img>").attr("src", "https://images2.minutemediacdn.com/image/fetch/w_736,h_485,c_fill,g_auto,f_auto/https%3A%2F%2Fnetflixlife.com%2Ffiles%2Fimage-exchange%2F2022%2F04%2Fie_85541-1-850x560.jpeg"))).append($("<div>").attr("class", "cyrus-card-body").append($("<div>").attr("class", "cyrus-card-title").append($("<h4>").attr("class", "cyrus-card-title").html(item.title))).append($("<div>").attr("class", "cyrus-card-description").append($("<div>").attr("class", "cyrus-card-description-info").append($("<span>").html((seasonsTotal > 0 ? seasonsTotal + " Temporadas, " : "") + videosTotal + " Vídeos"))).append($("<div>").attr("class", "cyrus-card-description-type").append($("<span>").html("Série")))));
                            }
                            else if (item instanceof Video) {
                                let duration = Math.round(item.duration / 60);
                                let title = (item.season !== null ? "Temporada " + ((_a = item.season) === null || _a === void 0 ? void 0 : _a.numeration) : "") + (item.numeration !== null ? "Episódio " + item.numeration : "") + ((item.season !== null || item.anime !== null) ? " - " : "") + item.title;
                                card.append($("<a>").attr("class", "cyrus-card-link").attr("href", "www.google.com").attr("title", item.title)).append($("<div>").attr("class", "cyrus-card-image").append($("<img>").attr("class", "c-opacity-70").attr("src", "https://images2.minutemediacdn.com/image/fetch/w_736,h_485,c_fill,g_auto,f_auto/https%3A%2F%2Fnetflixlife.com%2Ffiles%2Fimage-exchange%2F2022%2F04%2Fie_85541-1-850x560.jpeg")).append($("<div>").attr("class", "cyrus-card-duration").append($("<span>").html(duration + "m"))).append($("<i>").attr("class", "fa-solid fa-play cyrus-card-center"))).append($("<div>").attr("class", "cyrus-card-body").append($("<div>").attr("class", "cyrus-card-description").append($("<div>").attr("class", "cyrus-card-description-info").append($("<span>").html((_b = item.anime) === null || _b === void 0 ? void 0 : _b.title)))).append($("<div>").attr("class", "m-0 cyrus-card-title").append($("<h4>").attr("class", "cyrus-card-title").html(title))).append($("<div>").attr("class", "m-0 cyrus-card-description").append($("<div>").attr("class", "cyrus-card-description-type").append($("<span>").html("Episódio")))));
                            }
                            else {
                                continue;
                            }
