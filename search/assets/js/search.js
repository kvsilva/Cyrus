                    ...
                    if (result.data) {
                        ...
                        for (let i = 0; i < 3; i++) {
                            ...
                            if (result.data.length - 1 >= i) {
                                ...
                                if (item instanceof Anime) {
                                    ...
                                    card.append($("<a>").attr("class", "cyrus-card-link").attr("href", (Routing === null || Routing === void 0 ? void 0 : Routing.episode) + "?episode=" + item.id).attr("title", item.title)).append($("<div>").attr("class", "cyrus-card-image").append($("<img>").attr("class", "c-opacity-70").attr("src", (_c = item.thumbnail) === null || _c === void 0 ? void 0 : _c.path)).append($("<div>").attr("class", "cyrus-card-duration").append($("<span>").html(duration + "m"))).append($("<i>").attr("class", "fa-solid fa-play cyrus-card-center"))).append($("<div>").attr("class", "cyrus-card-body").append($("<div>").attr("class", "cyrus-card-description").append($("<div>").attr("class", "cyrus-card-description-info").append($("<span>").html((_d = item.anime) === null || _d === void 0 ? void 0 : _d.title)))).append($("<div>").attr("class", "m-0 cyrus-card-title").append($("<h4>").attr("class", "cyrus-card-title").html(title))).append($("<div>").attr("class", "m-0 cyrus-card-description").append($("<div>").attr("class", "cyrus-card-description-type").append($("<span>").html("Episódio")))));
                                }
                                else {
                                    continue;
                                }
                        ...
                        if (series.length > 0) {
                            ...
                            for (let i = 0; i < series.length; i++) {
                                ...
                                for (let v = 0; v < videosFromSeasons.length; v++)
                                    videosTotal += videosFromSeasons[v].length;
                                card.append($("<a>").attr("class", "cyrus-card-link").attr("href", (Routing === null || Routing === void 0 ? void 0 : Routing.animes) + "?anime=" + item.id).attr("title", item.title)).append($("<div>").attr("class", "cyrus-card-image-catalog").append($("<img>").attr("src", (_e = item.profile) === null || _e === void 0 ? void 0 : _e.path))).append($("<div>").attr("class", "cyrus-card-body").append($("<div>").attr("class", "cyrus-card-title").append($("<h4>").attr("class", "cyrus-card-title").html(item.title))).append($("<div>").attr("class", "cyrus-card-description").append($("<div>").attr("class", "cyrus-card-description-info").append($("<span>").html((seasonsTotal > 0 ? seasonsTotal + (seasonsTotal == 1 ? " Temporadas, " : " Temporadas, ") : "") + videosTotal + (videosTotal == 1 ? " Video" : " Vídeos")))).append($("<div>").attr("class", "cyrus-card-description-type").append($("<span>").html("Série")))));
                        ...
                        if (videos.length > 0) {
                            ...
                            for (let t = 0; t < videosType.length; t++) {
                                ...
                                for (let i = 0; i < videosByType.length; i++) {
                                    ...
                                    card.append($("<a>").attr("class", "cyrus-card-link").attr("href", (Routing === null || Routing === void 0 ? void 0 : Routing.episode) + "?episode=" + item.id).attr("title", item.title)).append($("<div>").attr("class", "cyrus-card-image-flex").append($("<img>").attr("class", "c-opacity-70").attr("src", (_g = item.thumbnail) === null || _g === void 0 ? void 0 : _g.path)).append($("<div>").attr("class", "cyrus-card-duration").append($("<span>").html(duration + "m"))).append($("<i>").attr("class", "fa-solid fa-play cyrus-card-center"))).append($("<div>").attr("class", "cyrus-card-body").append($("<div>").attr("class", "cyrus-card-description").append($("<div>").attr("class", "cyrus-card-description-info").append($("<span>").html((_h = item.anime) === null || _h === void 0 ? void 0 : _h.title)))).append($("<div>").attr("class", "m-0 cyrus-card-title").append($("<h4>").attr("class", "cyrus-card-title").html(title))).append($("<div>").attr("class", "m-0 cyrus-card-description").append($("<div>").attr("class", "cyrus-card-description-type").append($("<span>").html("Episódio")))));
