...
class Dubbing {
    ...
    function __construct(int $id = null, array $flags = array(self::NORMAL)) {
        ...
        if($id != null && $this->database != null){
            ...
            if($query->num_rows > 0){
                ...
                $this->path = $row["path"];
                $this->available = $this->available != "" ? Availability::getAvailability($row["available"]) : null;
            } else {
    ...
    public static function find(int $id = null, int $video = null, Availability $available = Availability::AVAILABLE, string $sql = null, array $flags = [self::NORMAL]) : array{
        ...
        while($row = $query->fetch_array()){
            $result[] = new Dubbing($row["id"], $flags);
        }
        return $result;
    }
     ...
     * @return Dubbing
     */
    public function setPath(mixed $path): Dubbing
    {
        $this->path = $path;
        return $this;
    }
