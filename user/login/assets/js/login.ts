...
$(document).ready(function () {
    $("#username-email").on('input', function () {
        if ($.trim(<string>$(this).val()).length > 0 && $.trim(<string>$("#password").val()).length > 0) {
            $("#execute").prop("disabled", false);
        } else $("#execute").prop("disabled", true);
    });
    $("#password").on('input', function () {
        if ($.trim(<string>$(this).val()).length > 0 && $.trim(<string>$("#username-email").val()).length > 0) {
            $("#execute").prop("disabled", false);
        } else $("#execute").prop("disabled", true);
    });
    $("#execute").click(function(){
       let username_email : string | number | string[] | undefined = $("#username-email").val();
