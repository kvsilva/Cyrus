...
//@ts-ignore
window.query = (search, status = null) => query(search, status);
function query(search = "", status = null) {
    if (search.length > 0)
        search = "%" + search + "%";
    API.requestService("Session", "getSession", {}, []).then((result) => {
        ...
        if (result.status) {
            if ("data" in result) {
                let user = (_a = result.data[0]) === null || _a === void 0 ? void 0 : _a.id;
                let formData = {
                    subject: search,
                    user: user
                };
